...
def init_model(config, checkpoint=None, device='cuda:0'):
    ...
    print("MinkSingleStage3DDetector=", model)
    return model


def inference_detector(model, pcd, data_np):
...
def show_det_result_meshlab(data,
    ...
    if 'pts_bbox' in result[0].keys():
        ...
        pred_labels = result[0]['pts_bbox']['labels_3d'].numpy() #luowei add
    else:
        pred_bboxes = result[0]['boxes_3d'].tensor.numpy()
        pred_scores = result[0]['scores_3d'].numpy()
        pred_labels = result[0]['labels_3d'].numpy()  #luowei add

    # filter out low score bboxes for visualization
    if score_thr > 0:
        inds = pred_scores > score_thr
        pred_bboxes = pred_bboxes[inds]
        pred_labels = pred_labels[inds] #luowei add
        pred_scores = pred_scores[inds] #luowei add
