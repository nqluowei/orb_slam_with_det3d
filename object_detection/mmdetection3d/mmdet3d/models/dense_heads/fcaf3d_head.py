...
class FCAF3DHead(BaseModule):
    ...
    def _make_up_block(in_channels, out_channels):
        ...
        """
        return nn.Sequential(
            ME.MinkowskiGenerativeConvolutionTranspose( #反卷积
                in_channels,
                out_channels,
                kernel_size=2,
                stride=2,
    ...
    def forward(self, x):
        ...
        for i in range(len(inputs) - 1, -1, -1):
            if i < len(inputs) - 1:
                ...
                x = inputs[i] + x
                x = self._prune(x, prune_score) #简枝
    ...
    def forward_test(self, x, input_metas):
        ...
        """
        center_preds, bbox_preds, cls_preds, points = self(x) #Neck+Head
        return self._get_bboxes(center_preds, bbox_preds, cls_preds, points,
                                input_metas)#post process
    ...
    def _single_scene_multiclass_nms(self, bboxes, scores, input_meta):
        ...
        for i in range(n_classes): #注意，遍历每个类别，在一个类别内部做NMS！！！所以table和desk不会因为重合取消
        ...
        if len(nms_bboxes):
            ...
            new_nms_ids = nms3d(nms_bboxes, nms_scores, self.test_cfg.iou_thr)
            nms_bboxes = nms_bboxes[new_nms_ids]
            nms_scores = nms_scores[new_nms_ids]
            nms_labels = nms_labels[new_nms_ids]
        else:
